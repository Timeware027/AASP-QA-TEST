...
async function createPersonRequired(driver) {
    try {
        ...
        await allure.step("Preenchendo o campo 'Nome'", async (ctx) => {
            try {
                ...
                await nomeInput.sendKeys('Vinícius Nascimento Borges');
                await ctx.parameter("Status", "200");
            } catch (error) {
                await ctx.parameter("Status", "400");
        ...
        await allure.step("Clicando no botão de registro", async (ctx) => {
            try {
                ...
                await driver.wait(until.elementLocated(By.css('.jss2858.jss2861.jss2867')));

                const mensagem = await driver.findElement(By.css('.jss2858.jss2861.jss2867 #message-id'));
