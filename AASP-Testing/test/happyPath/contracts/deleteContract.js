...
async function deleteContract(driver) {
    await allure.step("Navegando até o contrato", async (ctx) => {
        try {
            await driver.get('https://aaspgerenciador.aasp.org.br/contratos/17000/editar');
            await driver.sleep(2000);

            await ctx.parameter("Status", "200");
            await ctx.parameter("Descrição", "Navegado até o contrato com sucesso");
        } catch (error) {
            await ctx.parameter("Status", "400");
            console.error("Erro ao navegar até o contrato:", error);
            await takeScreenshot(driver, "Erro ao navegar até o contrato");
            throw error;
        }
    });
    ...
    await allure.step("Verificando mensagem de sucesso", async (ctx) => {
        try {
            ...
            if (textoMensagem === 'Excluído com sucesso') {
